...
#
 ...
 # set -vx
  ...
  [[ ! -d  $RestoreDir ]] && mkdir -p $DirBackupList
...
#------------------------------------------------------------------------
{
  ...
  #
     ...
     # ControlFile=$(ls -1 ${OFA_DB_BKP}/${SID_SOURCE}/bck_control_file_*-00.ctl)
     ControlFile=$(ls -1 ${OFA_DB_BKP}/${SID_SOURCE}/controlfile_*.after.ctl)

    if [ -z "${ControlFile}" ];then
        LogError "Control file not found ${OFA_DB_BKP}/${SID_SOURCE}/bck_control_file_*after.ctl"
	exit 1
    fi
...
EOF
  ...
  # 
    ...
    let STEP+=1 && STEP_LABEL="Step: $STEP/$STEPS - Start restore of database $SID_SOURCE" && LogCons "$STEP_LABEL"
    ...
    rman < $RmanDatabaseRestore 2>&1 | LogStdIn
     ...
     ErrorMessage=$(grep RMAN- $LOGFILE)
